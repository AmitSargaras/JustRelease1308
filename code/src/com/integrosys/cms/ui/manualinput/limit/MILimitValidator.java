...
public class MILimitValidator {
	public static ActionErrors validateMILimit(ActionForm aForm, Locale locale) {
		...
		try {
			...
			if(!"WS_SCOD_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_SCOD_FAC_EDIT_INITIAL".equalsIgnoreCase(lmtForm.getEvent())
					&& !"WS_ADHOC_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_ADHOC_FAC_EDIT_INITIAL".equalsIgnoreCase(lmtForm.getEvent())) {
			if(lmtForm!=null && ("WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) || "WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())) ){
			...
			if(lmtForm!=null && !"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent())){ 
				if (!(errorCode = Validator.checkString(lmtForm.getFacilityName(), true, 1, 100)).equals(Validator.ERROR_NONE)) { 
					 errors.add("facilityName", new ActionMessage("error.string.mandatory", "1", "100")); 
				...
				// Adhoc Faciluty
			if("".equals(lmtForm.getAdhocFacility()) || null == lmtForm.getAdhocFacility()) {
					errors.add("adhocFacilityError", new ActionMessage("error.string.mandatory")); 
					}
			
						
				if("Y".equals(lmtForm.getAdhocFacility()) ){
					
					if( "".equals(lmtForm.getAdhocLastAvailDate()) ||   null == lmtForm.getAdhocLastAvailDate() ) {
					errors.add("adhocLastAvailDateError", new ActionMessage("error.string.mandatory")); 
					}
					if (!(errorCode = Validator.checkDate(lmtForm.getAdhocLastAvailDate(), false, locale)).equals(Validator.ERROR_NONE)) {
						errors.add("adhocLastAvailDateError", new ActionMessage("error.date.format"));
					}
					if("".equals(lmtForm.getMultiDrawdownAllow()) ||  null == lmtForm.getMultiDrawdownAllow() ) {
						errors.add("multiDrawdownAllowError", new ActionMessage("error.string.mandatory")); 
						}
					
					if("".equals(lmtForm.getAdhocTenor()) || null == lmtForm.getAdhocTenor() ) {
						errors.add("adhocTenorError", new ActionMessage("error.string.mandatory")); 
						}
					/*if (!(errorCode = Validator.checkNumber(lmtForm.getAdhocTenor(), false, 0, 999))
							.equals(Validator.ERROR_NONE)) {
						errors.add("adhocTenorError", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.NUMBER, errorCode),
								"0", 100.00 + ""));
					}*/
					
					if (!("".equals(lmtForm.getAdhocTenor())) && !(isNumeric(lmtForm.getAdhocTenor())) || lmtForm.getAdhocTenor().length() >3 ) {
						errors.add("adhocTenorError", new ActionMessage("error.adhocTenor.invalidNumbers"));
					}
					
					if("".equals(lmtForm.getAdhocFacilityExpDate()) || null == lmtForm.getAdhocFacilityExpDate()) {
						errors.add("adhocFacilityExpDateError", new ActionMessage("error.string.mandatory")); 
						}
					if (!(errorCode = Validator.checkDate(lmtForm.getAdhocFacilityExpDate(), false, locale)).equals(Validator.ERROR_NONE)) {
						errors.add("adhocFacilityExpDateError", new ActionMessage("error.date.format"));
					}
				}
				
				if("".equals(lmtForm.getGeneralPurposeLoan()) || null == lmtForm.getGeneralPurposeLoan()) {
					errors.add("generalPurposeLoanError", new ActionMessage("error.string.mandatory")); 
					}
			}
			 
			if(!"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())) {
			 ...
			 if(lmtForm.getLimitType().equals("Yes")  && "No".equals(lmtForm.getGuarantee())) {
				 ...
				 if (!(errorCode = Validator.checkString(lmtForm.getMainFacilityId(), true, 1, 200)).equals(Validator.ERROR_NONE)) { 
					 errors.add("subFacilityName", new ActionMessage("error.string.mandatory")); 
				}
			 }
...
//				 }else{
				 if (!(errorCode =Validator.checkAmount(lmtForm.getAdhocLmtAmount(), true,1, IGlobalConstant.MAXIMUM_ALLOWED_AMOUNT_38_2,IGlobalConstant.DEFAULT_CURRENCY, locale))
							.equals(Validator.ERROR_NONE)) {
...
//			 			requiredSecCov = new BigDecimal(lmtForm.getRequiredSecCov().replaceAll(",", "")).add(new BigDecimal(lmtForm.getAdhocLmtAmount().replaceAll(",", "")));
				...
				if( totalReleasedAmount.compareTo(releasableAmount) > 0){
					...
					if(!"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())){
...
//			}
	...
	// commented by bharat to temporary remove totalsancamount validation	on 07/05/2012
		 ...
		 */
		if(flag && lmtForm.getLimitType().equals("No") 
				&& !"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent())
				&& !"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())
				){
	...
	//----------------------------commented by bharat-----end-----------------------------			
			...
			if(!"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent())
			   && !"create".equals(lmtForm.getEvent()) && !"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())){
			...
			if(!"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent())
					&& !"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())){
				if(StringUtils.isBlank(lmtForm.getRiskType())) {
					errors.add("riskType", new ActionMessage("error.string.mandatory"));
				}	
			}	
			
			if((StringUtils.isBlank(lmtForm.getTenor()) && StringUtils.isNotBlank(lmtForm.getTenorUnit())) ||
				...
				if(!"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent())
					...
					!(errorCode = Validator.checkNumber(lmtForm.getTenor(), false, 0, IGlobalConstant.MAXIMUM_ALLOWED_VALUE_3_2)).equals(Validator.ERROR_NONE)){
				errors.add("tenor", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.NUMBER, errorCode), "0", IGlobalConstant.MAXIMUM_ALLOWED_VALUE_3_2_STR));
			}
			else if(StringUtils.isNotBlank(lmtForm.getTenor()) && lmtForm.getTenor().contains("."))
			{
				errors.add("tenor", new ActionMessage("error.tenor.three.digit"));
			}
			
			if (!(errorCode = Validator.checkString(lmtForm.getTenorDesc(), false, 1, 255)).equals(Validator.ERROR_NONE)) {
				errors.add("tenorDesc", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1", "255"));
			}
			
			if (!(errorCode = Validator.checkNumber(lmtForm.getMargin(), false, 0, IGlobalConstant.MAXIMUM_ALLOWED_VALUE_3_2,3,locale)).equals(Validator.ERROR_NONE)) {
				errors.add("margin", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.NUMBER, errorCode), "0", IGlobalConstant.MAXIMUM_ALLOWED_VALUE_3_2_STR));
			}
			
			if(StringUtils.isNotBlank(lmtForm.getMargin())) {
				if (!(errorCode = Validator.checkNumber(lmtForm.getMargin(), true, 0, 100.00, 3,locale)).equals(Validator.ERROR_NONE)) {
					errors.add("margin",  new ActionMessage(
			...
			String strDate= formatter.format(DateUtil.getDate());
			String optionDate,loanavailabilitydate;
				if (lmtForm.getOptionDate() != null && !lmtForm.getOptionDate().isEmpty()) {
					optionDate = formatter.format(DateUtil.convertDate(locale,
							lmtForm.getOptionDate()));
					if (!strDate.equals(optionDate)) {
						if (StringUtils.isNotBlank(lmtForm.getOptionDate())
								&& DateUtil
										.convertDate(locale,
												lmtForm.getOptionDate())
										.before(DateUtil.getDate())) {
							errors.add("optionDate", new ActionMessage(
									"error.date.before.notallowed"));
						}
					}
				}
				if (lmtForm.getLoanAvailabilityDate() != null && !lmtForm.getLoanAvailabilityDate().isEmpty()) {
					if (isValidDateHyphen(lmtForm.getLoanAvailabilityDate().trim())) {
					loanavailabilitydate = formatter.format(DateUtil.convertDate(locale, lmtForm.getLoanAvailabilityDate()));
					if (!strDate.equals(loanavailabilitydate)) {
						if (StringUtils.isNotBlank(lmtForm.getLoanAvailabilityDate()) && 
								DateUtil.convertDate(locale, lmtForm.getLoanAvailabilityDate()).before(DateUtil.getDate())) {
							errors.add("loanAvailabilityDate", new ActionMessage("error.date.before.notallowed"));
						}
					}
				}
			}}
...
//SCCOD validation//
			...
			/*if ("Y".equals(lmtForm.getProjectFinance()) || "Y".equals(lmtForm.getProjectLoan())) {
				if ("1".equals(lmtForm.getLelvelDelaySCOD())
						//&& "WS_SCOD_FAC_EDIT".equals(lmtForm.getEvent())
						) {
					if (StringUtils.isBlank(lmtForm.getInfaProjectFlag())) {
						errors.add("infaProjectFlag", new ActionMessage("error.string.mandatory"));
					} else if (!("Y".equals(lmtForm.getInfaProjectFlag()) || "N".equals(lmtForm.getInfaProjectFlag()))) {
						errors.add("infaProjectFlag", new ActionMessage("error.infraFlag.invalid"));
					}
				}
			}*/
...
//					|| "WS_SCOD_FAC_EDIT_INITIAL".equalsIgnoreCase(lmtForm.getEvent())){
			...
			if("Y".equals(lmtForm.getProjectFinance()) && "N".equals(lmtForm.getProjectLoan()))	{
			...
			*/
			...
			/*if(("".equals(lmtForm.getWhlmreupSCOD()) || lmtForm.getWhlmreupSCOD() == null) 
				...
				if(!"create".equals(lmtForm.getEvent()) &&  !"WS_SCOD_FAC_EDIT_INITIAL".equals(lmtForm.getEvent())){
					...
					if(StringUtils.isNotBlank(lmtForm.getActualCommOpsDate())) {
						
						Date scodDate = DateUtil.convertDate(locale, lmtForm.getScodDate());
						Date acodDate = DateUtil.convertDate(locale, lmtForm.getActualCommOpsDate());
						boolean isAcodDateError = Boolean.FALSE;
						
						if(acodDate.before(scodDate)) {
							isAcodDateError = Boolean.TRUE;
						}
						else if("1".equals(lmtForm.getLelvelDelaySCOD()) && StringUtils.isNotBlank(lmtForm.getEscodLevelOneDelayDate())) {
							Date escodDate1 = DateUtil.convertDate(locale, lmtForm.getEscodLevelOneDelayDate());
							if(acodDate.before(escodDate1))
								isAcodDateError = Boolean.TRUE;
						}
						else if("2".equals(lmtForm.getLelvelDelaySCOD()) && StringUtils.isNotBlank(lmtForm.getEscodLevelSecondDelayDate())) {
							Date escodDate2 = DateUtil.convertDate(locale, lmtForm.getEscodLevelSecondDelayDate());
							if(acodDate.before(escodDate2))
								isAcodDateError = Boolean.TRUE;
						}
						else if("3".equals(lmtForm.getLelvelDelaySCOD()) && StringUtils.isNotBlank(lmtForm.getEscodLevelThreeDelayDate())) {
							Date escodDate3 = DateUtil.convertDate(locale, lmtForm.getEscodLevelThreeDelayDate());
							if(acodDate.before(escodDate3))
								isAcodDateError = Boolean.TRUE;
						}	
						
						if(isAcodDateError)
							errors.add("actualCommOpsDate", new ActionMessage("error.date.before.notallowed.acodDate"));
					}
				}
		...
		if("3".equals(lmtForm.getLelvelDelaySCOD()) && "Y".equals(lmtForm.getReasonBeyondPromoterLevel3Flag())) {
			...
			if(!"WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())
				&&	!"WS_SCOD_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_SCOD_FAC_EDIT_INITIAL".equalsIgnoreCase(lmtForm.getEvent()) 
					&& !"WS_ADHOC_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()) && !"WS_ADHOC_FAC_EDIT_INITIAL".equalsIgnoreCase(lmtForm.getEvent()) 		
			...
			if(PropertyManager.getValue("new.cam.webservice.mandatory.field.flag").equals("OFF") && ("WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent()) || "WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())  || "WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent()))) {
				...
				if(StringUtils.isEmpty(lmtForm.getClauseAsPerPolicy())) {
					...
					if(isFieldExcluded) {
						if(IBankingArrangementFacExclusionConstant.NA.equals(lmtForm.getBankingArrangement()) 
							...
							errors.add("clauseAsPerPolicy", new ActionMessage("error.limit.clauseAsPerPolicy.invalid.na"));
						}
					}
					else {
			...
			if(!(("WS_FAC_CREATE".equalsIgnoreCase(lmtForm.getEvent())) || ("WS_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())) || ("WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())))) {
			...
			String isPrimary = "";
			String securityId = "";
			String drawingPowerAmt = "";
		//	int drawPowerAmt = 0;
		//	int releaseAmt = 0;
			
			
			String custId = lmtForm.getCustomerID();
			boolean flag1 = false;
			String[] arrString = new String[3];
			if("Y".equals(lmtForm.getIsDPRequired()) && "Y".equals(lmtForm.getIsDP())) {
				boolean securityLinked = false;
				if(lmtForm.getSecurityTypeSubType() != null && (lmtForm.getSecurityTypeSubType()).length > 0) {
					for(int i=0;i<lmtForm.getSecurityTypeSubType().length; i++) {
						securityData = (String)(lmtForm.getSecurityTypeSubType())[i];
						if(securityData.contains("General Charge") || securityData.contains("Stock and Receivable") ) {
							securityLinked= true;
							arrString = securityData.split(",");
							securityId = arrString[2];
							isPrimary = limitDao.getSecurityPriorityValue(securityId);
				...
				System.out.println("securityLinked="+securityLinked);
				if(securityLinked){
					System.out.println("inside if securityLinked="+securityLinked);
				}
				else {
					errors.add("securityLinkError", new ActionMessage("error.security.link"));
				}
				BigDecimal drawPowerAmt=BigDecimal.ZERO;
				BigDecimal releaseAmt =BigDecimal.ZERO;
				if(flag1 == true) {
					drawingPowerAmt = limitDao.getSecurityDrawingPower(securityId,custId);
					if( null != drawingPowerAmt) {
						 drawPowerAmt= MapperUtil.stringToBigDecimal(UIUtil.removeComma(drawingPowerAmt));
						 if(null == drawingPowerAmt || "" == drawingPowerAmt) {
							 drawPowerAmt=BigDecimal.ZERO;
						 }
					//	drawPowerAmt = Integer.parseInt(drawingPowerAmt);
					}
					if(null != lmtForm.getTotalReleasedAmount()) {
						 releaseAmt =MapperUtil.stringToBigDecimal(UIUtil.removeComma(lmtForm.getTotalReleasedAmount()));
						 if(null == releaseAmt ) {
							 releaseAmt=BigDecimal.ZERO;
						 }
					//	releaseAmt = Integer.parseInt(UIUtil.removeComma(lmtForm.getTotalReleasedAmount()));
					}
					if(null != drawingPowerAmt && null != releaseAmt ){
					if(releaseAmt.compareTo(drawPowerAmt)>0) {
						errors.add("TotalReleasedError", new ActionMessage("error.totalreleasedamount.bigger"));
					}
					}
			...
			if((lmtForm.getSecurityTypeSubType() != null && (lmtForm.getSecurityTypeSubType()).length > 0) && !"WS_NEW_FAC_EDIT".equalsIgnoreCase(lmtForm.getEvent())) {
				if(!"Y".equals(lmtForm.getIsSecured())) {
					errors.add("isSecured", new ActionMessage("error.securedcheckbox.value"));
				}				
	...
	public static ActionErrors validateXRef(ActionForm aForm, Locale locale) {
		String errorCode = null;
		ActionErrors errors = new ActionErrors();
		try {
			...
			String singaporeFacSystem=PropertyManager.getValue("singapore.systemName");
...
//			}
			...
			if(ASSTValidator.isValidAlphaNumStringWithSpace((xrefForm.getReleasedAmount().replaceAll(",", "")))) {
			 ...
			 if (! (errorCode = Validator.checkDate(xrefForm.getReleaseDate(), false, locale)).equals(Validator.ERROR_NONE)) {
				  errors.add("releaseDate", new	  ActionMessage("error.date.mandatory", "1", "256")); 
				  }
			 if ((xrefForm.getReleaseDate().length() > 0)
						&& DateUtil.convertDate(locale, xrefForm.getReleaseDate()).after(DateUtil.getDate()))
			 {
				 errors.add("releaseDate", new ActionMessage("error.date.future.notallowed"));
			 }
			 
					 
			 
			 if (! (errorCode = Validator.checkDate(xrefForm.getDateOfReset(), true, locale)).equals(Validator.ERROR_NONE)) {
				  errors.add("dateOfReset", new	  ActionMessage("error.date.mandatory", "1", "256")); 
				  }
				 ...
				 if(xrefForm.getIsPrioritySector()!=null && xrefForm.getIsPrioritySector().equalsIgnoreCase("Blank")){
					 if( "UBS-LIMITS".equalsIgnoreCase(xrefForm.getFacilitySystem())) {
						 System.out.println("****inside MiLimitValidator ****1752*******system is UBS-LIMITS and isprioritySector is blank so showing madatory validation ");
						 errors.add("isPrioritySector", new ActionMessage("error.string.mandatory")); 
					 }else {
			 ...
			 if (!(errorCode = Validator.checkString(xrefForm.getInterestRateType(), true, 1, 40))
					...
					if(xrefForm.getInterestRateType().equals("fixed") && xrefForm.getIntRateFix().equals("")){
						errors.add("interestRateType", new ActionMessage("error.string.mandatory"));
					}else if(xrefForm.getInterestRateType().equals("fixed") && !xrefForm.getIntRateFix().equals("")){
						if (!(errorCode = Validator.checkNumber(xrefForm.getIntRateFix(), false , 0, IGlobalConstant.MAXIMUM_ALLOWED_VALUE_PERCENTAGE,3,locale))
								.equals(Validator.ERROR_NONE)) {

							errors.add("intRateFix", new ActionMessage(
					...
					if(ASSTValidator.isValidAlphaNumStringWithSpace(xrefForm.getIntRateFix())) {
						errors.add("intRateFix", new ActionMessage("error.string.invalidCharacter"));
					}
					if(xrefForm.getInterestRateType().equals("floating")) {
						if(xrefForm.getIntRateFloatingType().equals("") || 
							xrefForm.getIntRateFloatingRange().equals("")  || xrefForm.getIntRateFloatingMarginFlag()== null || 
							xrefForm.getIntRateFloatingMarginFlag().equals("") || xrefForm.getIntRateFloatingMargin() == null ||
							xrefForm.getIntRateFloatingMargin().equals("") ){
						errors.add("interestRateType", new ActionMessage("error.string.mandatory"));
					}
...
//			 if (! (errorCode = Validator.checkDate(xrefForm.getLimitExpiryDate(), false, locale)).equals(Validator.ERROR_NONE)) {
//				  errors.add("limitExpiryDate", new	  ActionMessage("error.date.mandatory", "1", "256")); 
//				  }
//			 if ((xrefForm.getLimitExpiryDate().length() > 0)
//						&& DateUtil.convertDate(locale, xrefForm.getLimitExpiryDate()).after(DateUtil.getDate()))
//			 {
//				 errors.add("limitExpiryDate", new ActionMessage("error.date.future.notallowed"));
//			 }
			 ...
			 //main line code mandatory for subline
				...
				/*if(null==xrefForm.getIsDayLightLimitAvailable() || "".equals(xrefForm.getIsDayLightLimitAvailable())){
				 ...
				 if (! (errorCode = Validator.checkDate(xrefForm.getLimitStartDate(), true, locale)).equals(Validator.ERROR_NONE)) {
					  errors.add("limitStartDate", new	  ActionMessage("error.date.mandatory", "1", "256")); 
					  }
				 if ((xrefForm.getLimitStartDate().length() > 0)
							&& DateUtil.convertDate(locale, xrefForm.getLimitStartDate()).after(DateUtil.getDate()))
				 {
					 errors.add("limitStartDate", new ActionMessage("error.date.future.notallowed"));
				 }
			 ...
			 if ((xrefForm.getUncondiCancl() == null) || (xrefForm.getUncondiCancl().equals(""))) {
					errors.add("uncondiCancl", new ActionMessage("error.date.mandatory"));
				}
			 
			 if (!(errorCode = Validator.checkString(xrefForm.getInternalRemarks(), false, 1, 1000))
					 .equals(Validator.ERROR_NONE)) {
			...
			/* if("Yes".equalsIgnoreCase(xrefForm.getIntradayLimitFlag()))
			 ...
			 if (null!=xrefForm.getIntradayLimitExpiryDate() && (xrefForm.getIntradayLimitExpiryDate().length() > 0)
						&& DateUtil.convertDate(locale, xrefForm.getIntradayLimitExpiryDate()).before(DateUtil.getDate()))
			 {
				 errors.add("intradayLimitFutureDateError", new ActionMessage("error.date.before.notallowed"));
			 }
			...
			/* if("Yes".equalsIgnoreCase(xrefForm.getIntradayLimitFlag()))
			 ...
			 else if (null!=xrefForm.getDayLightLimit() && xrefForm.getDayLightLimit().length() > 53) 
			 {
				 errors.add("dayLightLimitLengthError", new ActionMessage("error.dayLightLimit.length.mandatory")); 
		      }
				 ...
				 for(int i=0;i<xrefForm.getDayLightLimit().length();i++)
				 {
					 ...
					 {
						 errors.add("dayLightLimitNumericError", new ActionMessage("error.dayLightLimit.numeric.mandatory")); 
						 break;
					 }
					 else if(alpha == '.')
					 {
			 ...
			 Calendar calendar = Calendar.getInstance();
			 calendar.add(Calendar.DATE, -1);
			 if(null!=xrefForm.getIdlEffectiveFromDate() && (!xrefForm.getIdlEffectiveFromDate().isEmpty())) {		 
			 ...
			 {
				 if(null != xrefForm.getIdlAmount() && !"".equals(xrefForm.getIdlAmount())) {
					 BigDecimal idlAmtcheck = new BigDecimal(xrefForm.getIdlAmount().replaceAll(",", ""));
					 BigDecimal checkAmt = new BigDecimal("0");
					 if( idlAmtcheck.compareTo(checkAmt) > 0) {
			 ...
			 String limitProfileId = xrefForm.getLimitProfileID();
			 LimitDAO limitDAO=new LimitDAO();
			 ...
			 if(null != xrefForm.getIdlAmount() && !"".equals(xrefForm.getIdlAmount())){
			 if(ASSTValidator.isValidAlphaNumStringWithSpace((xrefForm.getIdlAmount().replaceAll(",", "")))) {
			 ...
			 System.out.println("MiLimitValidator.java => 1915=>xrefForm.getIdlAmount()=>"+xrefForm.getIdlAmount()+"***xrefForm.getIdlEffectiveFromDate()=>"+xrefForm.getIdlEffectiveFromDate()+"**");
			 if((null != xrefForm.getIdlAmount() && !"".equals(xrefForm.getIdlAmount())) || (null!=xrefForm.getIdlEffectiveFromDate()  && !"".equals(xrefForm.getIdlEffectiveFromDate()))){
				 BigDecimal idlAmt =new BigDecimal("0");
				 if(null != xrefForm.getIdlAmount() && !"".equals(xrefForm.getIdlAmount())) {
					 idlAmt = new BigDecimal(xrefForm.getIdlAmount().replaceAll(",", ""));
					 System.out.println("MiLimitValidator.java => 1920=>idlAmt=>"+idlAmt+"**");
				 }
				 BigDecimal releasableAmt = new BigDecimal(xrefForm.getReleaseableAmountCheck().replaceAll(",", ""));
				 System.out.println("MiLimitValidator.java => 1923=>idlAmt=>"+idlAmt+"**");
			 	...
			 	System.out.println("MiLimitValidator.java => 1928=>xrefForm.getIdlAmount()=>"+xrefForm.getIdlAmount()+"**");
			 }
			 
			 if(singaporeFacSystem.equals(xrefForm.getFacilitySystem())) {
				 
				 if ((xrefForm.getLiabBranch() == null) || (xrefForm.getLiabBranch().equals(""))) {
						errors.add("liabBranch", new ActionMessage("error.string.mandatory"));
					}
				 
				 if ((xrefForm.getCurrency() == null) || (xrefForm.getCurrency().equals(""))) {
						errors.add("Currency", new ActionMessage("error.string.mandatory"));
					}
				 
				 if ((xrefForm.getLimitTenorDays() == null) || (xrefForm.getLimitTenorDays().equals(""))) {
						errors.add("limitTenorDays", new ActionMessage("error.string.mandatory"));
					}
				 ...
				 if ((xrefForm.getFreezLCBGDevoInvok() == null) || (xrefForm.getFreezLCBGDevoInvok().equals(""))) {
						errors.add("freezLCBGDevoInvok", new ActionMessage("error.string.mandatory"));
					}
				 
				 if ((xrefForm.getSecuredUnsecured() == null) || (xrefForm.getSecuredUnsecured().equals(""))) {
							errors.add("securedUnsecured", new ActionMessage("error.string.mandatory"));
				 }
				 
				 if ((xrefForm.getSblc() == null) || (xrefForm.getSblc().equals(""))) {
						errors.add("sblc", new ActionMessage("error.string.mandatory"));
				 }
				 
				 if ((xrefForm.getRealEstate() == null) || (xrefForm.getRealEstate().equals(""))) {
						errors.add("realEstate", new ActionMessage("error.string.mandatory"));
				 }
				 
				 if ((xrefForm.getLimitStatus() == null) || (xrefForm.getLimitStatus().equals(""))) {
						errors.add("limitStatus", new ActionMessage("error.string.mandatory"));
				 }
				 
				 if ((xrefForm.getTakeoverLoan() == null) || (xrefForm.getTakeoverLoan().equals(""))) {
						errors.add("takeoverLoan", new ActionMessage("error.string.mandatory"));
				 }
				 
				 if ((xrefForm.getSblcsbbgAvailable() == null) || (xrefForm.getSblcsbbgAvailable().equals(""))) {
						errors.add("sblcsbbgAvilable", new ActionMessage("error.string.mandatory"));
					}
								 
				 if (!AbstractCommonMapper.isEmptyOrNull(xrefForm.getSblcSbbgPercent() ) && !isNumeric(xrefForm.getSblcSbbgPercent())){ 
						errors.add("sblcSbbgPercent", new ActionMessage("error.string.invalidNumbers"));
					}
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getSblcSbbgPercent(), false, 1, 5))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("sblcSbbgPercent", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "5"));
				 }
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getIssuingBank(), false, 1, 255))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("issuingBank", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "255"));
				}
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getSblcApplicant(), false, 1, 255))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("sblcApplicant", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "255"));
				}
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getSblcSbbgAmount(), false, 1, 255))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("sblcSbbgAmount", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "255"));
				}				 
				
				 if (!(errorCode = Validator.checkString(xrefForm.getSblcno(), false, 1, 255))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("sblcno", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "255"));
				}
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getOrderId(), false, 1, 255))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("orderId", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "255"));
				}
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getLtv(), false, 1, 200))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("ltv", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "200"));
				}

				 if (!(errorCode = Validator.checkString(xrefForm.getInvestmentId(), false, 1, 100))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("investmentId", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "100"));
				}
				 
				 if ((xrefForm.getNbfc() == null) || (xrefForm.getNbfc().equals(""))) {
						errors.add("nbfc", new ActionMessage("error.string.mandatory"));
					}
				 
				 if ((xrefForm.getTakeoverCases() == null) || (xrefForm.getTakeoverCases().equals(""))) {
						errors.add("takeOverCases", new ActionMessage("error.string.mandatory"));
					}
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getFreezeReason(), false, 1, 200))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("freezeReason", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "200"));
				 }
				 
				 if (!(errorCode = Validator.checkString(xrefForm.getClosureReason(), false, 1, 200))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("closureReason", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "200"));
				 }
				 
				 if ((xrefForm.getCreditOwner() == null) || (xrefForm.getCreditOwner().equals(""))) {
						errors.add("creditOwner", new ActionMessage("error.string.mandatory"));
					}
				 
				 if ((xrefForm.getRuleId() == null) || (xrefForm.getRuleId().equals(""))) {
						errors.add("ruleId", new ActionMessage("error.string.mandatory"));
					}
				 
				 if (!AbstractCommonMapper.isEmptyOrNull(xrefForm.getMainLineCode()) && !isNumeric(xrefForm.getMainLineCode())){ 
						errors.add("mainLineCode", new ActionMessage("error.string.invalidNumbers"));
					}
				 if (!(errorCode = Validator.checkString(xrefForm.getMainLineCode(), false, 1, 20))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("mainLineCode", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "20"));
				}
				/* if (!AbstractCommonMapper.isEmptyOrNull(xrefForm.getDescription()) && !isNumeric(xrefForm.getMainLineCode())){ 
						errors.add("description", new ActionMessage("error.string.mandatory"));
					}*/
				 if (!(errorCode = Validator.checkString(xrefForm.getDescription(), false, 1, 1020))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("description", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "1020"));
				}
				 if (!(errorCode = Validator.checkString(xrefForm.getInternalRemarks(), false, 1, 4000))
						 .equals(Validator.ERROR_NONE)) {
						 errors.add("internalRemarks", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "4000"));
				}
				 if(ASSTValidator.isValidInternalRemarks(xrefForm.getInternalRemarks())) {
					...
					if (null != xrefForm.getLimitTenorDays() && !"".equalsIgnoreCase(xrefForm.getLimitTenorDays())) {
					 ...
					 if(!AbstractCommonMapper.isEmptyOrNull(xrefForm.getAvailableAmount())){
							if(ASSTValidator.isValidAlphaNumStringWithSpace(UIUtil.removeComma(xrefForm.getAvailableAmount()))) {
								errors.add("availableAmount", new ActionMessage("error.string.invalidCharacter"));
							}
						}
					 
					 
					 if (!(errorCode = Validator.checkAmount(xrefForm.getAvailableAmount().replaceAll(",", ""), false, 0,
							IGlobalConstant.MAXIMUM_ALLOWED_AMOUNT_24_3,IGlobalConstant.DEFAULT_CURRENCY, locale)).equals(Validator.ERROR_NONE)) {
							errors.add("availableAmount", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.NUMBER, errorCode), 
									"0",IGlobalConstant.MAXIMUM_ALLOWED_AMOUNT_24_3_STR));
					 }

					 if(!AbstractCommonMapper.isEmptyOrNull(xrefForm.getApprovedAmount())){
							if(ASSTValidator.isValidAlphaNumStringWithSpace(UIUtil.removeComma(xrefForm.getApprovedAmount()))) {
								errors.add("approvedAmount", new ActionMessage("error.string.invalidCharacter"));
							}
					 }
					 					 
					 if (!(errorCode = Validator.checkNumber(xrefForm.getApprovedAmount().replaceAll(",", ""), false, 0,
					         IGlobalConstant.MAXIMUM_ALLOWED_AMOUNT_24_3)).equals(Validator.ERROR_NONE)) {
					     errors.add("approvedAmount", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.NUMBER, errorCode),
					             "0", IGlobalConstant.MAXIMUM_ALLOWED_AMOUNT_24_3_STR));
					 }

					 if (!(errorCode = Validator.checkString(xrefForm.getCategory(), false, 1, 200))
							 .equals(Validator.ERROR_NONE)) {
							 errors.add("category", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
							 "200"));
					}
					 if (!(errorCode = Validator.checkString(xrefForm.getUserDefinedStatusLabel(), false, 1, 200))
							 .equals(Validator.ERROR_NONE)) {
							 errors.add("userDefinedStatus", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
							 "200"));
					}
					 
					 if (null != xrefForm.getLimitRestriction() && !"".equalsIgnoreCase(xrefForm.getLimitRestriction())) {
							if (!(StringUtils.isNumeric(xrefForm.getLimitRestriction()))) 
			              		 errors.add("limitRestriction", new ActionMessage("error.amount.format"));
						}
					 
					 if (!(errorCode = Validator.checkString(xrefForm.getSblcsbbgIssuingBank(), false, 1, 200))
							 .equals(Validator.ERROR_NONE)) {
							 errors.add("sblcsbbgIssuingBank", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
							 "200"));
					}
					 if (!(errorCode = Validator.checkString(xrefForm.getSblcsbbgNumber(), false, 1, 200)) .equals(Validator.ERROR_NONE)) {
							 errors.add("sblcsbbgNumber", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
							 "200"));
					}
					 if (!(errorCode = Validator.checkString(xrefForm.getSecurityTaken(), false, 1, 200)) .equals(Validator.ERROR_NONE)) {
						 errors.add("securityTaken", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "200"));
				}
					 if (!(errorCode = Validator.checkString(xrefForm.getFinanceforQualityAcquisition(), false, 1, 200)) .equals(Validator.ERROR_NONE)) {
						 errors.add("financeforQualityAcquisition", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "200"));
				}
					 if (!(errorCode = Validator.checkString(xrefForm.getLoanToValue(), false, 1, 200)) .equals(Validator.ERROR_NONE)) {
						 errors.add("loanToValue", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "200"));
				}
					 
					 if (!(errorCode = Validator.checkString(xrefForm.getInvestmentAccountNumber(), false, 1, 100)) .equals(Validator.ERROR_NONE)) {
						 errors.add("investmentAccountNumber", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "100"));
				}else  if(!AbstractCommonMapper.isEmptyOrNull(xrefForm.getOrderNumber()) && AbstractCommonMapper.isEmptyOrNull(xrefForm.getInvestmentAccountNumber())) {
					 errors.add("investmentAccountNumber",  new ActionMessage("error.string.mandatory")); 
				 }
					 if (!(errorCode = Validator.checkString(xrefForm.getOrderNumber(), false, 1, 100)) .equals(Validator.ERROR_NONE)) {
						 errors.add("orderNumber", new ActionMessage(ErrorKeyMapper.map(ErrorKeyMapper.STRING, errorCode), "1",
						 "100"));
				}else  if(!AbstractCommonMapper.isEmptyOrNull(xrefForm.getInvestmentAccountNumber())  && AbstractCommonMapper.isEmptyOrNull(xrefForm.getOrderNumber())) {
						 errors.add("orderNumber",  new ActionMessage("error.string.mandatory")); 
					 }
					 
//				if(StringUtils.isBlank(xrefForm.getLineNo()))
//					errors.add("lineNo", new ActionMessage("error.string.mandatory"));

				/*if(StringUtils.isNotBlank(xrefForm.getProjectLoan()) && !StringUtils.isAlphanumeric(xrefForm.getProjectLoan()))
					errors.add("projectLoan", new ActionMessage("error.invalid.field.value"));
				
				if(StringUtils.isNotBlank(xrefForm.getSyndicateLoanLine()) && !StringUtils.isAlphanumeric(xrefForm.getSyndicateLoanLine()))
					errors.add("syndicateLoanLine", new ActionMessage("error.invalid.field.value"));
				
				if(StringUtils.isNotBlank(xrefForm.getEcbLoan()) && !StringUtils.isAlphanumeric(xrefForm.getEcbLoan()))
					errors.add("ecbLoan", new ActionMessage("error.invalid.field.value"));	
				*/

				if(StringUtils.isNotBlank(xrefForm.getSecured()) && !StringUtils.isAlpha(xrefForm.getSecured()))
					errors.add("secured", new ActionMessage("error.invalid.field.value"));
			 }
